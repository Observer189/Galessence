...
public class SceneLoader : MonoBehaviour
{
    ...
    {
        yield return LoadLoadingSceneAsync(unloadableScenes, loadingScene);
        textLoader = GameObject.Find("TextLoader").GetComponentInChildren<TextMeshProUGUI>();
        yield return LoadLoadableSceneAsync(loadingScene, loadableScene);
    }

    IEnumerator LoadLoadingSceneAsync(string[] unloadableScenes, string loadableScene)
    {
    ...
    IEnumerator LoadLoadableSceneAsync(string loadableScene, string loadingScene)
    {
        ...
        while (!f.isDone)
        {
            if (f.progress >= 0.90f && !f.allowSceneActivation)
            {
                ...
                if (Input.anyKeyDown)
                {
                    ...
                    Debug.Log("AllowLoading!");
                    SceneManager.SetActiveScene(SceneManager.GetSceneByName(loadableScene));
